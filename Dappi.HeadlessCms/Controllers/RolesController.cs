...
public class RolesController : ControllerBase
{
    ...
    public async Task<IActionResult> GetRoles([FromQuery] string searchTerm = "")
    {
        ...
        if (!string.IsNullOrWhiteSpace(searchTerm))
        {
            query = query.Where(role =>
                role.Name != null && role.Name.ToLower().Contains(searchTerm)
            );
        }

        var totalCount = query.Count();
        var roles = query.OrderBy(role => role.Name).ToList();
        ...
        var response = new { Total = totalCount, Data = roleDtos };
    ...
    public async Task<IActionResult> CreateRole([FromBody] CreateRoleDto dto)
    {
        ...
        return Ok(new RoleDto { Id = role.Id, Name = role.Name ?? roleName });
    }
