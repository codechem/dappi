...
[ApiController]
public class UpdateAppDbContextController(DbContextEditor dbContextEditor , DomainModelEditor domainModelEditor) : ControllerBase
{
    ...
    public async Task<IActionResult> UpdateDbContext()
    {
        try
        {
            var entitiesToBeRegistered = (await domainModelEditor.GetDomainModelEntityInfosAsync()).ToList();
