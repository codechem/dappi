...
namespace Dappi.HeadlessCms.Core.Extensions
{
    ...
    {
        public static AccessorListSyntax WithGetAndSet(
            SyntaxKind getKind = SyntaxKind.GetAccessorDeclaration,
        ...
        )
        {
            return SyntaxFactory.AccessorList(
                SyntaxFactory.List(
                    [
                        SyntaxFactory
                            .AccessorDeclaration(getKind)
                            .WithSemicolonToken(SyntaxFactory.Token(SyntaxKind.SemicolonToken)),
                        SyntaxFactory
                            .AccessorDeclaration(setKind)
                            .WithSemicolonToken(SyntaxFactory.Token(SyntaxKind.SemicolonToken)),
                    ]
                )
            );
        ...
        public static AttributeListSyntax WithCcControllerAttribute(
            List<CrudActions>? crudActionsList
        )
        {
            List<AttributeArgumentSyntax> arguments = [];
            var attribute = SyntaxFactory.Attribute(
                SyntaxFactory.ParseName(CcControllerAttribute.ShortName)
            );
            ...
            arguments.AddRange(
                crudActionsList.Select(a =>
                    SyntaxFactory.AttributeArgument(
                        SyntaxFactory.ParseExpression($"{nameof(CrudActions)}.{a}")
                    )
                )
            );
        ...
        public static PropertyDeclarationSyntax IdentityProperty()
        {
            return SyntaxFactory
                .PropertyDeclaration(SyntaxFactory.IdentifierName("Guid"), "Id")
                ...
                .AddAttributeLists(
                    ...
                    SyntaxFactory.AttributeList(
                        SyntaxFactory.SingletonSeparatedList(
                            SyntaxFactory
                                .Attribute(SyntaxFactory.ParseName("DatabaseGenerated"))
        ...
        public static PropertyDeclarationSyntax WithRelationAttribute(
            this PropertyDeclarationSyntax property,
            DappiRelationKind? relationType = null,
        ...
        )
        {
            var attributeList = SyntaxFactory.AttributeList(
                SyntaxFactory.SingletonSeparatedList(
                    SyntaxFactory
                        .Attribute(SyntaxFactory.ParseName(DappiRelationAttribute.ShortName))
                        .AddArgumentListArguments(
                            SyntaxFactory.AttributeArgument(
                                SyntaxFactory.ParseExpression(
                                    $"{nameof(DappiRelationKind)}.{relationType}"
                                )
                            ),
                            SyntaxFactory.AttributeArgument(
                                SyntaxFactory.ParseExpression($"typeof({relatedType})")
                            )
                        )
                )
            );
        ...
        private static PropertyDeclarationSyntax WithRegexAttribute(
            this PropertyDeclarationSyntax property,
        ...
        public static PropertyDeclarationSyntax WithRegularExpressionAttribute(
            this PropertyDeclarationSyntax property,
            string regex
        )
        {
            return property.WithRegexAttribute(
                "RegularExpression",
                SyntaxFactory.AttributeArgument(SyntaxFactory.ParseExpression($"@\"{regex}\""))
            );
        }

        public static PropertyDeclarationSyntax WithFutureDateAttribute(
            this PropertyDeclarationSyntax property
        )
        {
        ...
        public static PropertyDeclarationSyntax WithLengthAttribute(
            this PropertyDeclarationSyntax property,
            string? minLength,
            string? maxLength
        )
        {
            ...
            if (hasMin && hasMax)
            {
                ...
                lengthAttribute = lengthAttribute.WithArgumentList(
                    SyntaxFactory.AttributeArgumentList(
                        SyntaxFactory.SeparatedList(lengthArguments)
                    )
                );
            ...
            attribute = attribute.WithArgumentList(
                SyntaxFactory.AttributeArgumentList(
                    SyntaxFactory.SingletonSeparatedList(
                        SyntaxFactory.AttributeArgument(
                            SyntaxFactory.ParseExpression(attributeValue)
                        )
                    )
                )
            );
        ...
        public static PropertyDeclarationSyntax WithRangeAttribute(
            this PropertyDeclarationSyntax property,
            string? minValue,
            string? maxValue,
            string propertyType
        )
        {
            ...
            else
            {
                minExpression = SyntaxFactory.LiteralExpression(
                    ...
                    SyntaxFactory.Literal(
                        minDouble.Value.ToString(System.Globalization.CultureInfo.InvariantCulture),
                        minDouble.Value
                    )
                );
            }
            ...
            else
            {
                maxExpression = SyntaxFactory.LiteralExpression(
                    SyntaxKind.NumericLiteralExpression,
                    SyntaxFactory.Literal(
                        maxDouble.Value.ToString(System.Globalization.CultureInfo.InvariantCulture),
                        maxDouble.Value
                    )
                );
            }
        ...
        public static MemberDeclarationSyntax[] GeneratePropertiesFromType(Type type)
        {
            ...
            var typeMembers = type.GetProperties(
                BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly
            );

            foreach (var member in typeMembers)
            {
                var newMember = GenerateDynamicProperty(
                        member.PropertyType.GetDisplayName(),
                        member.Name,
                        !member.IsNullable()
                    )
                    .AddModifiers(SyntaxFactory.Token(SyntaxKind.PublicKeyword))
        ...
        public static PropertyDeclarationSyntax GenerateDynamicProperty(
            string propertyType,
            string propertyName,
