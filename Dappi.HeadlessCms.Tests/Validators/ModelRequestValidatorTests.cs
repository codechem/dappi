...
public class ModelRequestValidatorTests
{
    ...
    [Theory]
    [MemberData(nameof(ModelRequestValidatorTestData.InvalidModelNameTestCases), MemberType = typeof(ModelRequestValidatorTestData))]
    public void Should_have_error_for_invalid_ModelName(ModelRequest model, Expression<Func<ModelRequest, object?>> propertyExpression, string expectedErrorMessage)
    {
        var result = _validator.TestValidate(model);

        result.ShouldHaveValidationErrorFor(propertyExpression).WithErrorMessage(expectedErrorMessage);
    }

    [Theory]
    [MemberData(nameof(ModelRequestValidatorTestData.ValidModelNameTestCases), MemberType = typeof(ModelRequestValidatorTestData))]
    public void Should_not_have_error_for_valid_ModelName(ModelRequest model, Expression<Func<ModelRequest, object?>> propertyExpression)
    {
        var result = _validator.TestValidate(model);
